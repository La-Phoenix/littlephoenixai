...
export const ConversationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const selectConversation = useCallback(async (conversationId?: string, skipFetch?: boolean) => {
    ...
    try {
      ...
      console.log('selectConversation: activeConversationId set to', conversationId ?? (response[0]?.conversationId ?? null));
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        console.log('sendMessage: sending with activeConversationId', activeConversationId, 'isNewConv', isNewConv);
